...
def extract_random(full_imgs,full_masks, patch_h,patch_w, N_patches, inside=True):
    ...
    # img_shape1 = full_imgs.shape[3]
    # img_shape2 = full_imgs.shape[4]
    pre_patch = np.zeros((1,1)+full_imgs.shape[-2:])
    pre_mask = np.zeros((1,1)+full_masks.shape[-2:])
    for i in range(full_imgs.shape[0]):  #loop over the full images
        k=0
        while k <patch_per_img:
            # x_center = random.randint(0+int(patch_w/2),img_w-int(patch_w/2))
            border_reject = 16 # Don't sample too close to border 
            x_center = random.randint(int(img_w/border_reject)+int(patch_w/2),img_w-int(img_w/border_reject)-int(patch_w/2))
            # print "x_center " +str(x_center)
            # y_center = random.randint(0+int(patch_h/2),img_h-int(patch_h/2))
            y_center = random.randint(int(img_h/border_reject)+int(patch_h/2),img_h-int(img_w/border_reject)-int(patch_h/2))
            # print "x_center " +str(x_center) + ", y_center " +str(y_center)
            #check whether the patch is fully contained in the FOV
            if inside==True:
                if is_patch_inside_FOV(x_center,y_center,img_w,img_h,patch_h)==False:
                    continue
            # Random shear to image before zooming
            pre_patch[0,0], pre_mask[0,0] = random_rot_shear_trans(full_imgs[i,0],full_masks[i,0])
            # patch = full_imgs[i,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)]
            # patch_mask = full_masks[i,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)]
            patch = pre_patch[0,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)]
            patch_mask = pre_mask[0,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)]
            patches[iter_tot]=patch
            patches_masks[iter_tot]=patch_mask
            iter_tot +=1   #total
            k+=1  #per full_img
    patches, patches_masks = random_flips(patches,patches_masks)
    # patches, patches_masks = random_rot_shear_trans(patches,patches_masks)
    return patches, patches_masks

def random_flips(img_patches,img_masks):
    th1 = np.random.randint(0,2,size=img_patches.shape[0])
    th2 = np.random.randint(0,2,size=img_patches.shape[0])

    for idx in range(img_patches.shape[0]):
        img = img_patches[idx,0]
        mask = img_masks[idx,0]
        if th1[idx] == 1:
            # print('flip vertically',idx)
            img = np.flipud(img)
            mask = np.flipud(mask)
            img_patches[idx,0] = img # Note: idx,*0* assumes grayscale
            img_masks[idx,0] = mask
        if th2[idx] == 1:
            # print('flip horizontally',idx)
            img = np.fliplr(img)
            mask = np.fliplr(mask)
            img_patches[idx,0] = img # Note: idx,*0* assumes grayscale
            img_masks[idx,0] = mask

    return img_patches,img_masks

from skimage.transform import warp, AffineTransform
from skimage import transform
def random_rot_shear_trans(img,mask):
    # for idx in range(img_patches.shape[0]):
    # img = img_patches[idx,0]
    # mask = img_masks[idx,0]
    rot = np.random.uniform(0,0.25*np.pi) # Random angle between 0 and 1 rad
    shear = None # np.random.uniform(-0.5,0.5) # Random shear between 0 and 0.5 rad
    # Shifting to zero, rotating, and shifting back
    shift_y, shift_x = (np.array(img.shape)-1) / 2.
    tf_rotate = transform.SimilarityTransform(rotation=rot)
    tf_shift = transform.SimilarityTransform(translation=[-shift_x, -shift_y])
    tf_shift_inv = transform.SimilarityTransform(translation=[shift_x, shift_y])
    tform = AffineTransform(scale=None, rotation=None, shear=shear, translation=None) 
    #https://stackoverflow.com/questions/25895587/python-skimage-transform-affinetransform-rotation-center
    img = transform.warp(img, (tf_shift + (tf_rotate + tf_shift_inv)).inverse, order = 3)
    mask = transform.warp(mask, (tf_shift + (tf_rotate + tf_shift_inv)).inverse, order = 3)
   # shear in rad
    img = warp(img, tform.inverse, output_shape=img.shape)
...
def is_patch_inside_FOV(x,y,img_w,img_h,patch_h):
    ...
    R_inside = 140 - int(patch_h * np.sqrt(2.0) / 2.0) #radius is 270 (from DRIVE db docs), minus the patch diagonal (assumed it is a square #this is the limit to contain the full patch in the FOV
    radius = np.sqrt((x_*x_)+(y_*y_))
    # print(x, x_,y,y_,R_inside,radius)
    if radius < R_inside:
        return True
    else:
        return False
...
def is_patch_inside_FOV_new(x,y,img_w,img_h,patch_h):
    x_ = x - int(img_w/2) # origin (0,0) shifted to image center
    y_ = y - int(img_h/2)  # origin (0,0) shifted to image center
    R_inside = img_w/2 #92 - int(patch_h * np.sqrt(2.0) / 2.0) #radius is 270 (from DRIVE db docs), minus the patch diagonal (assumed it is a square #this is the limit to contain the full patch in the FOV
    radius = np.sqrt((x-x_)**2 + (y-y_)**2) #np.sqrt((x_*x_)+(y_*y_))
    print(x, x_,y,y_,R_inside,radius)
    if radius < R_inside:
        return True
    else:
